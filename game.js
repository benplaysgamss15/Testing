...
function saveGame() {
    // DO NOT SAVE IF WE ARE IN CO-OP! Protects the solo save.
...
function stashSoloData() {
    G.soloStash = {
        level: G.level, wheat: G.wheat, player: JSON.parse(JSON.stringify(G.player)),
        playerHp: G.playerHp, playerShield: G.playerShield, discovered: JSON.parse(JSON.stringify(G.discovered)),
...
function restoreSoloData() {
    if (G.soloStash) {
        G.level = G.soloStash.level; G.wheat = G.soloStash.wheat; G.player = G.soloStash.player;
        G.playerHp = G.soloStash.playerHp; G.playerShield = G.soloStash.playerShield; G.discovered = G.soloStash.discovered;
        G.volcanoTimer = G.soloStash.volcanoTimer; G.megaTimer = G.soloStash.megaTimer; G.megaOnLand = G.soloStash.megaOnLand;
        G.soloStash = null;
    }
}

function bondWithPartner(id, name) {
    stashSoloData(); // Save solo progress!
...
function breakCoop(reason) {
    ...
    restoreSoloData(); // Retrieve solo progress!
