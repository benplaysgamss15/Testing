...
function resize(){
    ...
    canvas.width = Math.min(window.innerWidth, 1280); canvas.height = Math.min(window.innerHeight, 800);
}
resize(); window.addEventListener('resize', resize);

const TS = 48; const WS = 160; const LERP = 0.1;
...
function getOC(cId) { return PALETTES.find(p=>p.id===cId)?.oc || PALETTES[0].oc; }
...
const GCLR=['#4a7c3f','#528942','#3d6b35','#6a9c5f','#508240']; const FCLR=['#2d5a27','#3a6b33','#254c20','#1e4518']; const WCLR=['#1a6b8a','#1e7a9f','#157a9a','#0d6080','#246090']; const SCLR=['#c8a85a','#d4b46a','#bc9e4e','#c4a456'];
const VGCLR=['#3a2e28','#42352e','#332722','#4a3a31','#3b2b25']; const VFCLR=['#221612','#2c1c17','#1a100d','#261814']; const VWCLR=['#8a251a','#9f2b1e','#9a2215','#80190d','#902214']; const VSCLR=['#5a3a2a','#664433','#4d3020','#5c3d2b']; const VRED=['#ff2a00','#e62600','#ff3300','#cc2200'];

const G = {
    state:'intro', level: 1, wheat:60, tick:0, cam:{x:0,y:0},
    ...
    custPart: 'body', playerHp:80, playerShield: 0, lastDamageTick: 0, discovered:{raptor:true}, wilds:[],
    ...
    soloStash: null, // <--- STASH VARIABLE TO HOLD SOLO PROGRESS

    keys:{}, joy:{on:false,sx:0,sy:0,dx:0,dy:0}, btns:[], encCd:0, idxPage:0, mx:-1,my:-1, particles:[], swapCd:0, cheatsActive:false,  
    peer: null, conns: {}, isHost: false, peerId: null, otherPlayers: {}, username: null, sessionTimer: null, 
    volcanoTimer: 10800, volcanoActive: 0, camShake: 0, hazards:[], megaTimer: 3600, megaOnLand: false
};

let syncInterval = null; let chatFadeTimer = null;

function pDino(){ return DINOS[G.player.dk]; }
function pMaxHp(){ let m = Math.floor((pDino().hp + G.player.upg.hp*28) * R_MULT[pDino().rarity]); return G.coop.partnerId ? Math.floor(m * 0.9) : m; }
function pAtk(){ let a = Math.floor((pDino().atk + G.player.upg.atk*5) * R_MULT[pDino().rarity]); return G.coop.partnerId ? Math.floor(a * 0.9) : a; }
